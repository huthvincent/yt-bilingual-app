...
| `config.py` | 所有配置：`.env` 加载、数据目录（HISTORY/SUBTITLES/SENTENCES）、安全开关、`MODEL_CATALOG`（可选 DeepSeek 模型与价格）、`VOCAB_LEVELS`（词汇水平→prompt 描述） |
| `llm.py` | DeepSeek OpenAI 兼容客户端（`chat_complete`，默认关掉 thinking） |
